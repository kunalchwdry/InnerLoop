...
function CommunityPage() {
  ...
  const [newPost, setNewPost] = useState({ type: 'discussion', title: '', content: '', tags: [], community_id: null, visibility: 'public' });
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      const postData = {
        ...newPost,
        tags: newPost.tags.filter(t => t.trim()),
      ...
      setNewPost({ type: 'discussion', title: '', content: '', tags: [], community_id: null, visibility: 'public' });
        ...
        for (const member of members) {
          if (member.user_id !== user.id && member.notify_new_posts) {
            await createNotification({
              ...
              content: `${user.full_name || 'Someone'} posted: ${created.title}`,
              type: 'info',
...
function CommunitySidebar({ user, myCommunities, activeTab, setActiveTab, onClose }) {
  const navItems = [
    { id: 'home', label: 'Home', icon: Home },
    { id: 'communities', label: 'Communities', icon: Users },
    { id: 'posts', label: 'All Posts', icon: MessageSquare },
    { id: 'help', label: 'Help Someone', icon: HelpCircle },
...
// Import AnimatePresence for page transitions
